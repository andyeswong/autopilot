...
export class ManagerAgentPrompt {
  ...
  8. ACTION SEQUENCING:
     ...
     - EFFICIENCY: When filling forms, batch ALL visible form fields together in a single action sequence before clicking submit. Don't fill one field at a time - fill multiple fields consecutively.
     - Example EFFICIENT form filling: [fillInput index 1, fillInput index 2, fillInput index 3, fillInput index 4, clickElement submit]
     - Example INEFFICIENT (avoid): [fillInput index 1] then wait, then [fillInput index 2] then wait...
     - Chain actions where nothing changes on the page: saving, extracting data, toggling checkboxes, filling multiple form fields.
