...
export class ManagerAgent {
  ...
  async executeTask(task: Task) {
    ...
    for (const [i, action] of task.actions.entries()) {
      try {
        // Smart DOM change detection: skip for consecutive form fills
        const currentAction = action.data.name;
        const previousAction = i > 0 ? task.actions[i - 1].data.name : null;
        
        // Only check DOM changes if:
        // - Not a fillInput after another fillInput (form filling optimization)
        // - Previous action was a click or scroll (likely causing page changes)
        const shouldCheckDomChange = i > 0 && !(
          currentAction === "fillInput" && previousAction === "fillInput"
        );
        
        if (shouldCheckDomChange && (await this.didDomStateChange())) {
        ...
        // Shorter delay for form filling, longer for clicks/scrolls
        const delayTime = currentAction === "fillInput" ? 300 : this.msDelayBetweenActions;
  ...
  private async executeAction(action: TaskAction) {
    ...
    switch (action.data.name) {
      case "clickElement":
        ...
        // Wait for potential navigation and page load after click
        await new Promise((resolve) => setTimeout(resolve, 2000));
        await this.browserService.getStablePage();
        // Additional wait for loading screens
        await new Promise((resolve) => setTimeout(resolve, 2000));
