...
export class ChromiumBrowser implements Browser {
  ...
  private minimumPageLoadTime: number = 2500;
  ...
  async fillInput(text: VariableString, coordinates: Coordinates) {
    ...
    // Clear existing content: select all (Ctrl+A) and then type new text
    await this.getPage().keyboard.press('Control+A');
    await this.getPage().keyboard.type(text.dangerousValue(), { delay: 30 });
  }
